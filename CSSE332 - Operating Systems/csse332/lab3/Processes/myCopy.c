...
int ReadFromFile(char fileName[], char *buffer){
  ...
  if (inputFile == NULL){
    fprintf(stderr, "Unable to open the IN file %s\n", fileName);
    exit(2);
  }
...
int WriteToFile(char fileName[], char *buffer, int numCharsToWrite){
  ...
  if(outFile == NULL){
    fprintf(stderr, "Unable to open the OUT file %s\n", fileName);
    exit(2);
  }
  int i = 0;
...
int main(int argc, char *argv[]){
  ...
  if (argc != 3){
    fprintf(stderr, "Number of arguments: %d  Usage:\n", argc);
    fprintf(stderr, ">>>  ./filecopy <source_file> <destination_file> \n");
    exit(1);
  }

  int charsCount =  ReadFromFile(argv[1], buffer);
