/* createChildAndWait.c 
   ...
   of the  parent's image. It is possible to overlay the image of the 
   child process with another image using the "exec" family of system 
   calls.  A process can use the "getpid" function to get its process id.  
...
int main(int argc, char *argv[]){
  ...
  if (argc!=3) {
    printf("./CreateAndWait2 <source-file> <destination-file>\n");
    exit(-1);
  }
  ...
  if (fork_pid==0) { /* If the current process is the child process. */
    ...
    /* wait(NULL); - Use NULL if you do not want to examine the
       ...
       wait() is a blocking call.  You can use waitpid() to esercise
